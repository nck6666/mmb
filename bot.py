...
token   = 'MTA2MjcyOTQyMjg0NDczMTUwMg.GBWdbk.Au_DfnO6O19-a43GeLu1Za5dQhwui2eex5TaIU' # paste your token here
...
async def add_buyer(ctx, buyer : int = None):
    if ctx.author.id not in admins:
        await ctx.send(f'Xin Lỗi, {ctx.author}, Bạn Không Phải Admin!')

    elif buyer in buyers:
        await ctx.send(f'{buyer} Đã Chiếm Được 1 Vị Trí!')

    elif buyer is None:
        await ctx.send('Xin Vui Lòng Mua Trước Khi Dùng!!')
...
async def delete_buyer(ctx, buyer : int = None):
    if ctx.author.id not in admins:
        ...
        await ctx.send('Vui Lòng Mua Trước Khi Dùng!!')
...
async def add_admin(ctx, admin : int = None):
    if ctx.author.id not in owners:
        ...
        await ctx.send(f'{admin} Đã Là QTV!')

    elif admin is None:
        await ctx.send('Cho Admin Xin Cái!!')
...
async def delete_admin(ctx, admin : int = None):
    if ctx.author.id not in owners:
        await ctx.send(f'Xin Lỗi, {ctx.author}, Nhưng Bạn Không Phải Chủ Sỡ Hữu!')

    elif admin not in admins:
        await ctx.send(f'{admin} Không Phải QTV')
...
\033[1;97m=============================================================
    ...
    if name == 'nt':
        ...
        system('clear')
    print(banner)
    print(f'\033[1;97mĐã Login Vào \033[1;96m{bot.user.name}')
